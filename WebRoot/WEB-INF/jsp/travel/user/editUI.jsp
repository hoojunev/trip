    ...
    var verifyFlag = false;
    	...
    	function doVerifyAccount(asyncFlag){
    		...
    		if(account != ""){
    			...
    			$.ajax({
    				url:"${basePath}travel/user_verifyAccount.action",
    				data:{"user.account":account, "user.id":"${user.id}"},
    				type:"get",//get,post
    				async: asyncFlag,//异步属性，默认为true；false表示为同步
    				success: function(msg){
    					if("true"!= msg){
    						alert("该帐号已经存在，请输入其它值。");
    						$("#account").focus();
    						verifyFlag = false;
    					} else {
    						verifyFlag = true;
...
<form id="form" name="form" action="${basePath }travel/user_edit.action" method="post" enctype="multipart/form-data">
    <div class="p_d_1">
        <div class="p_d_1_1">
