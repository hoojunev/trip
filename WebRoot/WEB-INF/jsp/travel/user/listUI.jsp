...
<head>
    ...
    <script type="text/javascript">
		...
		function doSelectAll(){
      	...
      	//新增
      	function doAdd(){
      		document.forms[0].action = "${basePath}travel/user_addUI.action";
      		document.forms[0].submit();
      	}
      	//编辑
      	function doEdit(id){
      		document.forms[0].action = "${basePath}travel/user_editUI.action?user.id=" + id;
      		document.forms[0].submit();
      	}
      	//删除
      	function doDelete(id){
      		document.forms[0].action = "${basePath}travel/user_delete.action?user.id=" + id;
      		document.forms[0].submit();
      	}
      	//批量删除
      	function doDeleteAll(){
      		document.forms[0].action = "${basePath}travel/user_deleteSelected.action";
      		document.forms[0].submit();
      	}
      	//导出用户列表
      	function doExportExcel(){
      		window.open("${basePath}travel/user_exportExcel.action");
      	}
      	//导入用户列表
      	function doImportExcel(){
      		document.forms[0].action = "${basePath}travel/user_importExcel.action";
      		document.forms[0].submit();
      	}
      	var list_url = "${basePath}travel/user_listUI.action";
      	//搜索
      	function doSearch(){
