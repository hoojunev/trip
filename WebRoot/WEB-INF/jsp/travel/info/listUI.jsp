...
<head>
	...
	function doSelectAll(){
  	...
  	//新增
  	function doAdd(){
  		document.forms[0].action = "${basePath}travel/info_addUI.action";
  		document.forms[0].submit();
  	}
  	//编辑
  	function doEdit(id){
  		document.forms[0].action = "${basePath}travel/info_editUI.action?info.infoId=" + id;
  		document.forms[0].submit();
  		
  	}
  	//删除
  	function doDelete(id){
  		document.forms[0].action = "${basePath}travel/info_delete.action?info.infoId=" + id;
  		document.forms[0].submit();
  	}
  	//批量删除
  	function doDeleteAll(){
  		document.forms[0].action = "${basePath}travel/info_deleteSelected.action";
  		document.forms[0].submit();
  	}
  	//异步信息发布
  	function doPublic(infoId, state){
  		//根据infoId修改数据库对应的信息的状态为state
  		$.ajax({
  			url:"${basePath}travel/info_publicInfo.action",
  			type:"get",
  			data:{"info.infoId":infoId, "info.state":state},
			success:function(msg){
				if("更新状态成功"==msg){
					//更新列表中的状态栏、操作栏的状态值
					if(state == 1){//说明当前信息的状态 发布，状态栏显示 发布，操作栏显示 停用
						$("#state_"+infoId).html("发布");
						$("#oper_"+infoId).html("<a href='javascript:doPublic(\"" + infoId + "\",0)'>停用</a>");
						
					} else {//说明当前信息的状态 停用，状态栏显示 停用，操作栏显示 发布
  	...
  	var list_url = "${basePath}travel/info_listUI.action";
  	//搜索
  	function doSearch(){
